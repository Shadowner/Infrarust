...
pub fn run(config_path: &Path) -> anyhow::Result<WizardOutcome> {
    ...
    if web_enabled {
        ...
        println!(
            ...
            console::style("plugins/admin_api/config.toml").dim()
        );
        println!();
        api_key = Some(key);
    }
...
fn write_files(config_path: &Path, settings: &WizardSettings) -> anyhow::Result<()> {
    ...
    if let Some(key) = &settings.api_key {
        let api_dir = PathBuf::from("plugins/admin_api");
        std::fs::create_dir_all(&api_dir)
            .context("failed to create plugins/admin_api directory")?;
        let api_toml = generate_admin_api_toml(key, &settings.web_bind, settings.web_port);
        std::fs::write(api_dir.join("config.toml"), api_toml)
...
fn generate_admin_api_toml(api_key: &str, bind: &str, port: u16) -> String {
    format!(
        r#"# Admin API configuration
# Generated by the quick-start wizard
