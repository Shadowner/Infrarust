...
impl Manager {
    ...
    pub async fn is_server_starting(&self, server_id: &str, manager_type: &ManagerType) -> bool {
        ...
        let result = self.starting_servers.lock().await.contains_key(&key);
        result
    }
    ...
    pub async fn mark_server_as_empty(
        ...
        // First check if server is starting without holding any other locks
        ...
        // Check if server is already marked for shutdown without holding other locks
        ...
        // Update time_since_empty in a separate block to release lock quickly
        {
            let mut time_since_empty = self.time_since_empty.lock().await;
        ...
        // Update shutdown_timers in a separate block
        {
            let mut shutdown_timers = self.shutdown_timers.lock().await;
    ...
    pub async fn remove_server_from_empty(
        ...
        // Release locks quickly by using separate blocks
        {
            let mut time_since_empty = self.time_since_empty.lock().await;
            if let Some(manager_map) = time_since_empty.get_mut(&manager_type) {
                manager_map.remove(server_id);
        ...
        // Call cancel_shutdown after releasing all locks
