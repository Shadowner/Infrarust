...
use infrarust_config::{ProxyConfig, ProxyMode, ServerConfig, validate_server_config};
...
fn write_files(config_path: &Path, settings: &WizardSettings) -> anyhow::Result<()> {
    ...
    if let (Some(domain), Some(address)) = (
        ...
        verify_server_config(&server_path)?;
        println!(
...
fn generate_server_toml(domain: &str, address: &str) -> anyhow::Result<String> {
    #[derive(serde::Serialize)]
    struct GeneratedServer<'a> {
        domains: [&'a str; 1],
        addresses: [&'a str; 1],
        proxy_mode: ProxyMode,
    }

    let body = toml::to_string_pretty(&GeneratedServer {
        domains: [domain],
        addresses: [address],
    ...
    Ok(format!(
        "# Server configuration generated by the quick-start wizard\n\
         # Full reference: https://infrarust.dev/v2/reference/config-schema\n\n{body}"
    ))
}

fn verify_server_config(path: &Path) -> anyhow::Result<()> {
    let content = std::fs::read_to_string(path)
        .with_context(|| format!("cannot read generated server config: {}", path.display()))?;
    let config: ServerConfig = toml::from_str(&content).with_context(|| {
        format!(
            "generated server config is invalid TOML: {}",
            path.display()
        )
    })?;
...
mod tests {
    ...
    fn assert_safe(input: &str) -> String {
        ...
        assert!(
            !out.is_empty()
                ...
                && !out.contains("..")
                && !out.starts_with('.'),
            "unsafe filename {out:?} from {input:?}"
        );
        out
    }

    #[test]
    fn sanitize_filename_neutralizes_traversal() {
        assert_eq!(assert_safe("../x"), "__x");
        assert_eq!(assert_safe("../../etc"), "____etc");
        assert_eq!(assert_safe("a/../b"), "a___b");
    }

    #[test]
    fn sanitize_filename_strips_leading_dots() {
        assert_eq!(assert_safe(".hidden"), "hidden");
        assert_eq!(assert_safe("..config"), "_config");
    }

    #[test]
    fn sanitize_filename_handles_empty_and_all_dot_input() {
        assert_eq!(sanitize_filename(""), "unnamed");
        assert_eq!(sanitize_filename("."), "unnamed");
        assert_eq!(assert_safe(".."), "_");
        assert_eq!(assert_safe("..."), "_.");
    }

    #[test]
    fn sanitize_filename_replaces_reserved_characters() {
        assert_eq!(assert_safe(r#"a/b\c:d*e?f"g<h>i|j"#), "a_b_c_d_e_f_g_h_i_j");
        assert_eq!(assert_safe(r"C:\evil"), "C__evil");
        assert_eq!(assert_safe("*.example.com"), "_.example.com");
    }

    #[test]
    fn sanitize_filename_keeps_normal_domains() {
        assert_eq!(assert_safe("mc.example.com"), "mc.example.com");
    }

    #[test]
    fn generated_server_toml_round_trips() {
        let toml_str = generate_server_toml("mc.example.com", "127.0.0.1:25565").unwrap();
    ...
    #[test]
    fn generated_server_toml_escapes_hostile_input() {
        let hostile = "evil\"]\naddresses = [\"attacker:25565";
        let toml_str = generate_server_toml(hostile, "127.0.0.1:25565").unwrap();
        let config: ServerConfig = toml::from_str(&toml_str).unwrap();
        assert_eq!(config.domains, [hostile]);
