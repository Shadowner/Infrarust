...
impl ApiProvider for LocalProvider {
    async fn get_server_status(
        ...
        // Get process state if running, otherwise use Stopped
        let state = if is_running {
            // Try to get actual state from process manager
            ...
            // Process is not running, so update our state to Stopped
            let mut server_states = self.server_states.lock().unwrap();
    ...
    async fn stop_server(&self, server_id: &str) -> Result<(), ServerManagerError> {
        ...
        // Update server state to Stopping
        {
            ...
            debug!("Set server state for '{}' to Stopping during shutdown", server_id);
        }
        ...
        // Update server state to Stopped
        {
            let mut server_states = self.server_states.lock().unwrap();
            server_states.insert(server_id.to_string(), ServerState::Stopped);
            debug!("Set server state for '{}' to Stopped after shutdown", server_id);
        }
