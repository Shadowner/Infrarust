    ...
    async fn list_configs(&self) -> String {
        ...
        for (config_id, config) in config_list {
            ...
            if config.domains.is_empty() {
                ...
                let domains: Vec<String> = config.domains.iter()
                    .map(|d| fmt::entity(d))
                    .collect();
                result.push_str(&format!("{}\n", domains.join(", ")));
            }
            ...
            if config.addresses.is_empty() {
                ...
                let addrs: Vec<String> = config.addresses.iter()
                    .map(|a| fmt::secondary(a))
                    .collect();
                result.push_str(&format!("{}\n", addrs.join(", ")));
            }
            
            let mode_str = config.proxy_mode.as_ref().map_or("Default", |mode| match mode {
            ...
            let proxy_str = config.send_proxy_protocol.map_or("Disabled", |v| if v { "Enabled" } else { "Disabled" });
            result.push_str(&format!(
                "  {}: {}\n", 
